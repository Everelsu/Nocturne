...
class User:
    ...
    async def send(self, payload: Dict) -> None:
        if self._websocket:
            try:
                ...
                LOGGER.warning(f"User ({self.id}) send failed: {e}")
    ...
    async def _listen(self) -> None:
        while True:
            ...
            try:
                ...
                LOGGER.warning(f"User ({self.id}) receive error: {e}")
                break

            try:
                await self.send_to_bot(json.loads(data))
            except Exception as e:
                LOGGER.warning(f"User ({self.id}) message handling error: {e}")
    ...
    async def connect(self, websocket: quart.Websocket) -> None:
        ...
        try:
            ...
            LOGGER.error(f"User ({self.id}) listener error: {e}")
        finally:
            await self.disconnect()
        ...
        for user_id, user in self._users.items():
            if user_id not in skip_users:
                try:
                    ...
                    LOGGER.warning(f"Guild ({self.id}) broadcast to user {user_id} failed: {e}")
    ...
    async def send(self, payload: Dict) -> None:
        if self.is_connected:
            try:
                ...
                LOGGER.warning(f"Bot ({self.id}) send failed: {e}")
    ...
    async def _listen(self):
        while True:
            try:
                ...
                LOGGER.warning(f"Bot ({self.id}) receive error: {e}")
                break
            ...
            if user_id := data.get("userId"):
                ...
                if user:
                    try:
                        ...
                        LOGGER.warning(f"Bot ({self.id}) forward to user {user_id} failed: {e}")

            elif guild:
                try:
                    await guild.broadcast(data)
                except Exception as e:
                    LOGGER.warning(f"Bot ({self.id}) guild broadcast failed: {e}")
...
class BotPool:
    ...
    async def create(cls, bot_id: str, websocket: quart.Websocket) -> None:
        ...
        try:
            ...
            LOGGER.error(f"Bot ({bot.id}) listener error: {e}")
            # Only disconnect if this websocket is still ours (prevent race condition)
            if bot._websocket is websocket:
                await bot.disconnect()
    ...
    async def broadcast(cls, data: Dict) -> None:
        for bot in cls._bots.values():
            try:
                ...
                LOGGER.warning(f"Broadcast to bot {bot.id} failed: {e}")
        ...
        for path in candidates:
            try:
                with open(path, "r") as file:
                    ...
                    if not section:
                        return data
                    # inherit top-level keys (e.g. client_id) if not in dashboard section
                    merged = dict(data)
