            ...
            try:
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status == 200:
                        ...
                        LOGGER.warning(f"Failed to download from {url}: {response.status}")
            except Exception as e:
                LOGGER.warning(f"Could not download GeoIP DB from {url}: {e}")

    LOGGER.warning("GeoIP database unavailable — country detection disabled.")