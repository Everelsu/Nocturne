...
const methods = {
    ...
    initBot: function (player, data) {
        ...
        bot.guildIds = new Set(data.guildIds || []);
    ...
    initPlayer: function (player, data) {
        ...
        player.currentGuildId = data.guildId;
        player.updateSelectedBotView();
...
class Player {
    ...
    updateSelectedBotView() {
        ...
        this.bots.forEach((value, key) => {
            if (value !== this.selectedBot) {
                const filterGuild = this.currentGuildId || this.guildId;
                if (filterGuild && value.guildIds && value.guildIds.size > 0 && !value.guildIds.has(filterGuild)) {
                    return;
                }
                $view.append(
    ...
    updateCurrentQueuePos(pos) {
        ...
        if (this.currentTrack && this.currentTrack.artworkUrl) {
            const PROXY_SOURCES = ["yandexmusic", "vkmusic", "deezer", "applemusic"];
            let colorThiefSrc = this.currentTrack.artworkUrl;
            if (PROXY_SOURCES.includes(this.currentTrack.source)) {
                colorThiefSrc = "/proxy/image?url=" + encodeURIComponent(colorThiefSrc);
            }

            const $colorImg = $("<img>").hide();
            $colorImg[0].crossOrigin = "anonymous";

            $colorImg.on("load", function () {
                try {
                    ...
                    if (
                        ...
                        adjustedColor = darkenColor(dominantColor, 0.3);
                    }
            ...
            $colorImg.on("error", function () { $(this).remove(); });
            $("body").append($colorImg);
            $colorImg.attr("src", colorThiefSrc);
        }
