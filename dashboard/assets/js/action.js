...
$(document).ready(function () {
    ...
    if (localStorage.getItem("corners") === "rounded") {
        $("html").addClass("rounded")
        $("#user-settings-page .settings-row input[data-id='toggle-corners']").prop("checked", true)
    }
    ...
    $(document).on("input propertychange change", ".settings-row [data-id]", function () {
        ...
        switch (true) {
            ...
            case pageContainerId.startsWith("user-settings-page"):
                if (dataId === "toggle-dark-mode") {
                    $("html").toggleClass("light-theme", !value)
                    localStorage.setItem("theme", !value ? "light" : "dark")
                } else if (dataId === "toggle-corners") {
                    $("html").toggleClass("rounded", value)
                    localStorage.setItem("corners", value ? "rounded" : "sharp")
                }
                break
        }
    })
    ...
    $(document).on("click", function (e) {
        ...
        if ($target.closest("#settings-page").length) {
            ...
            if ($targetSettings.hasClass("access-server-settings")) {
                player.currentGuildId = String(guildId);
